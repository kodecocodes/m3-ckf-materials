...
data class MovieViewState(val id: String, val title: String, val description: String, val imageRes: Int)
...
class HomeViewModel(
    ...
    private fun fetchMoviesByCategories() {
        viewModelScope.launch {
            movieRepository.moviesByCategories()
                .transform { moviesByCategories ->
                    val movieViewStates: Map<String, List<MovieViewState>> = moviesByCategories.mapValues { (category, movies) ->
                        movies.map { movie ->
                            ...
                            MovieViewState(id = movie.id, title = movie.title, description = description, imageRes = imageRes)
                        }
                    }
    ...
    private fun fetchFavouriteCategories() {
        viewModelScope.launch {
            movieRepository.fetchFavouriteCategories()
                .map { favouriteCategories -> favouriteCategories.map { MovieCategoryViewState(it.id, it.name) } }
